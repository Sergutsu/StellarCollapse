// Game scene: the match-4/Tetris board, active piece, HUD columns,
// scanner sweep, particle effects, title-star reactions, countdown
// overlays, and click-to-match input. Subscribes to GameState events
// and owns every Pixi node below boardRoot + the HUD container.
// Dependencies (Pixi app, scene + ui roots, shared panel/button
// helpers) are injected via constructor so the scene does not import
// from pixi-view (no circular imports).
//
// Contract (used by SceneManager + PixiView):
//   game.show()              -- makes roots visible (lazy-build)
//   game.hide()              -- hides roots (does NOT destroy)
//   game.layout(screen)      -- reserved for future viewport re-layout
//   game.tick(deltaMs)       -- drives scanner + pulse + star + tweens
//   game.destroy()           -- tears down all Pixi nodes
//   game.visible             -- scene manager contract
//
// Public helpers forwarded from PixiView:
//   game.createBoard()       -- rebuild cell pools for a new run
...
// stay put, and the title bar spans the top.
const HUD_W = 860;
const HUD_H = 820;
const TITLE_H = 80;
const COL_W = 210;
...
// Title-bar + panel text colors, also pulled from the DOM CSS.
...
export class GameScene {
    constructor({
        ...
        // Ongoing per-frame tween handles. Cleared on reset.
        this._tweens = new Set();
        // Free particle Graphics pool.
        this._particlePool = [];
        // Board cells currently rendering a bomb/snake.
        this._pulsingCells = new Set();
        // Monotonic clock for pulse sine phases.
        this._clockMs = 0;
        // Skip idle-pulse tickers on big boards.
        this._lowFx = false;

        // HUD containers/nodes; populated by _buildHud().
        this.hud = null;
        this.boardRoot = null;
        this._topControls = null;
        this._hudRoot = null;

        // Star-reaction tween state.
        this._starReactionTween = null;

        // Board frame + scanner state.
        ...
        // built (main.js primes sound state during init). Cache the
        // last value so _buildHud() can apply it on first show().
        this._pendingSoundEnabled = null;
        this._pendingTip = null;

        // Optional HUD level-name formatter (main.js wires this).
        this._levelInfoFor = null;

        // Visible state + built flag for scene-manager contract.
        this._visible = false;
        this._built = false;
    ...
    show() {
        ...
        if (this._hudRoot) this._hudRoot.visible = true;
        if (this.boardRoot) this.boardRoot.visible = true;
        if (this._topControls) this._topControls.visible = true;
        this._visible = true;
    }

    hide() {
        if (this._hudRoot) this._hudRoot.visible = false;
        if (this.boardRoot) this.boardRoot.visible = false;
        if (this._topControls) this._topControls.visible = false;
        this._visible = false;
    }

    layout(_screen) {
        // Board + HUD sit inside sceneRoot, which PixiView re-centers
        // on viewport resize. No per-scene layout pass needed today.
    }
    ...
    // ------------------------------------------------------------------
    // Lazy build: HUD + boardRoot created on first show().
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // Public API (forwarded from PixiView for zero main.js churn).
    // ------------------------------------------------------------------

    setLevelInfoFor(fn) {
        this._levelInfoFor = fn;
    }

    setTip(text) {
        if (this.hud?.tipText) {
            this.hud.tipText.text = text;
        } else {
            this._pendingTip = text;
        }
    }

    setSoundEnabled(enabled) {
        if (this.hud?.soundText) {
            this.hud.soundText.text = enabled ? '🔊 Sound ON' : '🔇 Sound OFF';
        } else {
            this._pendingSoundEnabled = enabled;
        }
    }
    ...
    // Build (or rebuild) the board cell pools for a new run. Called
    // on every game-started so each run gets a fresh cell pool sized
    ...
    // Preview grids are Pixi-owned (built inside _buildHud). Kept as
    ...
    // pre-allocated body / accent / outline / icon / pulse layer.
    // ------------------------------------------------------------------
    ...
    _redrawBoard() {
        if (!this.boardCells.length) return;
        const cols = this.state.cols;
        const rows = this.state.rows;
        ...
        for (let i = 0; i < count; i++) {
            ...
            this._addTween({
                ...
                update: (p) => {
                    g.clear();
                    const px = dx * p;
                    const py = dy * p + (p * p) * this.blockPx * 0.8;
                    g.x = cx + px;
                    g.y = cy + py;
    ...
    // ------------------------------------------------------------------
    // COLLAPSED special-cell countdown ring.
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // HUD updates (text mutations on state events).
    // ------------------------------------------------------------------
    ...
    _bindState() {
        ...
        s.on('match-detected', ({ cells, color, special }) => {
            ...
            // suppress the generic 'match' one.
    ...
    // ------------------------------------------------------------------
    // HUD construction. Runs once on first show(); panels reused for
    // every game.
    // ------------------------------------------------------------------
    ...
    _buildHud() {
        ...
        topControls.container.x = 12;
        topControls.container.y = 12;
        ...
        // Flush any values set before the HUD existed (main.js primes
        // sound state and tips during init, before the game scene is
        // shown for the first time).
        if (this._pendingSoundEnabled !== null) {
            this.hud.soundText.text = this._pendingSoundEnabled
                ? '🔊 Sound ON'
                : '🔇 Sound OFF';
            this._pendingSoundEnabled = null;
        }
        if (this._pendingTip !== null) {
            this.hud.tipText.text = this._pendingTip;
            this._pendingTip = null;
        }
    }
    ...
    // ------------------------------------------------------------------
    // Mode-dependent match control hint.
    // ------------------------------------------------------------------
