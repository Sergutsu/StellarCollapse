...
// Hub shell layout, resource chips, news ticker strings, and nav tabs
// now live in src/scenes/hub-scene.js -- extracted per ADR-0009 so the
// hub owns its own Pixi nodes + layout math. PixiView only keeps the
// in-game HUD constants below.
...
export class PixiView {
    constructor({ state, meta = null, elements }) {
        ...
        // Scene manager owns extracted Pixi scenes. ResultsScene and
        // HubScene have moved out; GameScene + tab-scenes + minigame
        // scenes stage in later PRs per ADR-0009. Registered in init()
        // once `uiRoot` exists.
        this._sceneMgr = new SceneManager();
        // Hub scene instance, created in init() once Pixi + uiRoot are
        // available. Holds the mission catalog, start-game callback,
        // and all hub Pixi nodes.
        this._hub = null;
    ...
    async init() {
        ...
        // Extracted scenes register here now that the Pixi app + uiRoot
        // exist. Shared panel + button helpers stay on PixiView for now
        // (hub + game still call them); ResultsScene gets bound
        // references so a later PR can move them into a ui-kit module
        // without touching this line.
        this._sceneMgr.register('results', new ResultsScene({
            app,
            uiRoot: this.uiRoot,
        ...
        // HubScene owns the "CHIEF DISPATCHER" main menu. Its Pixi tree
        // is built lazily on first show(), which happens right after
        // _buildHud below.
        ...
        this._buildHud();
        // Hub is the default boot screen, so eagerly build it now. The
        // scene manager's show() call is idempotent and triggers the
        // lazy-init inside HubScene itself.
        this._hub.show();
    ...
    _layoutStartScreen() {
        // Hub + Results + future scenes all re-center themselves on
        // viewport changes via the scene-manager fan-out.
        if (this.app) this._sceneMgr.layout(this.app.screen);
    }
    ...
    onStartGame(callback) {
        // Forward into HubScene: mission cards fire the callback from
        // _onMissionCardTapped. PixiView keeps the public wiring so
        // main.js does not need to know scenes exist.
