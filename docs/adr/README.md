...
- `0008-meta-state-persistence.md` — Player profile persists through a versioned `stellarVentureSaveV1` localStorage blob wrapped by `src/persistence.js`; every I/O path is non-throwing (SSR / private-mode / quota / unparseable / incompatible version all fall back to a starter profile). **Accepted.**
- `0009-scene-graph-extraction.md` — Split `PixiView` into scenes under `src/scenes/` + a tiny `SceneManager`, staged over three PRs (Results → Hub → Game). No framework; dependencies injected by hand at construction. Rejected an MVC + DI framework as disproportionate to the real problem ("one giant file", not "unwired deps"). **Accepted.**
- `0010-hub-tab-scenes.md` — Hub tab scenes: each bottom-nav tab with bespoke content becomes its own scene class under `src/scenes/tabs/`, hosted by `HubScene` (not the top-level `SceneManager`). Same duck-typed `show/hide/layout/destroy` contract; mutually exclusive in the hub's center slot; layout fans out on resize so hidden tabs don't flash. First extraction: STAR MAP. **Accepted.** (Refines ADR-0009.)
