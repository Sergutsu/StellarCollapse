...
- **Missions could brick themselves.** A manually-dispatched run permanently consumed its ship + crew ("On Mission" forever); after a reload they came back only as slow "recovery placeholder" jobs, eventually leaving zero free assets so DISPATCH stayed greyed out. Manual runs now free their ship + crew the moment you press CONTINUE on the results screen (`completeManualMission`), so the next DISPATCH is always available.

### Changed

- **Miner is reachable by default.** Dispatch mode now defaults to MANUAL (IDLE is opt-in), so pressing DISPATCH on a Mining mission instantly launches its minigame — the default-selected mission is `blocks-classic` "Core Drilling: Ironspan Flats", i.e. the Miner board. Stale planner hint text replaced.
- **RESET GAME button** in the hub top bar (left of the gear). Wipes the saved profile and reloads the page so a fresh run/mission board is one click away, even if save state wedged itself.
...
- **Research system improvements.** Multiple concurrent research projects (default 2 slots), proper Cancel + Resume with progress preservation, live progress in left column when on RESEARCH tab. Added `upgradeResearchSlots()` + basic Research Lab expansion in the BUILD/UPGRADE tab. 5 new MetaState tests.
- **Crew tab with hire/recruit.** New `CrewTab` scene renders a personnel roster (left) and detail card + recruit panel (right). Players can browse crew, view role/level/status, hire random candidates for 800 credits, or dismiss available crew for a partial refund. `MetaState.addCrew()` / `removeCrew()` mutations persist new hires across saves.
- **Market tab — resource exchange.** New `MarketTab` scene provides a two-way credits ↔ minerals exchange with preset trade amounts (10, 50, 100, 500). Buy rate: 5 credits/mineral; sell rate: 3 credits/mineral. Balances update live and buttons grey out when unaffordable.
- **Build/Upgrade → Shipyard rework.** The BUILD/UPGRADE tab has been rewritten as a ship-focused shipyard: left pane lists the current fleet with hull bars and status; right pane shows a detail card (REPAIR / DISASSEMBLE actions) and 6 ship blueprints (Scout, Defender, Harvester, Terraformer, Trader, Frigate). Building deducts minerals; disassembly returns 40% of build cost. `MetaState.addShip()` / `removeShip()` support dynamic fleet size.

### Changed

- **Header simplified.** O₂ and Fuel removed from the top-bar resource strip. Only Minerals, Credits, and Warp Cells are shown. The game economy is now built around the credits ↔ minerals exchange (market) rather than ship-maintenance resources.

### Previously Added

- **Defense mission mode (manual combat minigame).** New Space-Invaders / Breakout hybrid mission type integrated as a first-class game mode. Combat-type missions (`blocks-classic`, `blocks-collapsed`, and the hub's "Orbital Relay Defense Drill") now launch the defense minigame instead of the puzzle board. Player controls a paddle, bouncing ball destroys pixel-art invaders and a boss; towers, lasers, and power-ups provide strategic depth. Architecture follows the existing pure-state / scene-renderer split: `DefenseState` (pure, injectable RNG + scheduler), `DefenseScene` (Pixi rendering), `defense-input.js` (keyboard + mouse + touch). 42 new unit tests bring the total to 150. Hub dispatch flow routes `type === 'Combat'` missions to the defense mode; defense hub missions are now `manualReady: true`.
- **Idle fleet dispatch + mission planner (PRs #61-64).** The MISSIONS tab now splits into two screens — a **manual mission planner** (select a mission card, assign a ship + crew member, pick IDLE or MANUAL dispatch) and an **idle fleet panel** (left column) showing dispatched idle missions with progress, ETA, and ABORT/COMPLETE actions. Dispatch mode toggle (`IDLE` / `MANUAL`) sits inside the planner; idle missions cap at the visible concurrent limit (fleet size). Ship and crew availability is tracked per dispatch — deployed assets are marked unavailable and recovered on mission completion or abort. Mission types from the narrative catalog now map to playable catalog entries so DISPATCH actually launches the correct tier archetype.
- **BUILD/UPGRADE tab scene (PR #59).** New module `src/scenes/tabs/build-upgrade-tab.js` (~300 lines) renders a station diorama inside the hub's center panel when the **BUILD/UPGRADE** bottom-nav tab is active: isometric station silhouette with 4 interactive callout pins (Docking Arms, Reactor Spine, Sensor Crown, Fabrication Yard), a 3-slot build queue (one active build + 2 queued), and 2 available-upgrade cards with cost + effect preview. `BUILD/UPGRADE` is now unlocked — `HUB_TABS[2].locked = false`. The CTA is a stub; real build-queue logic and MetaState integration land under ROADMAP P5.
- **Tech-frame panel + chip render helpers (PR #57).** `pixi-ui-kit.js` gained `drawTechPanel`, `redrawTechPanel`, `drawTechChip`, and `redrawTechChip` — sharp-cornered, cut-corner sci-fi frame style used by the hub shell. `drawHologramPanel` / `redrawHologramPanel` now delegate to the tech-frame path so all panels share one visual language. Non-hub scenes (Results, Game) are unaffected — legacy hologram rendering was restored in PR #58.
- **Neon CTA button restyle (PR #60).** `buildStartButton` in `pixi-ui-kit.js` now renders with a colour-matched neon glow, frame stroke, and hover/active state transitions for a console-chrome aesthetic.
- **Mobile-responsive scaling + touch gestures (PRs #55-56).** The fixed 860×820 in-run HUD now viewport-fits via scene scaling; hub shell scale-fits below its desktop minimum so columns/nav/modal stop overlapping on phones. Touch gesture controls added to `src/input.js` — swipe left/right to move, swipe up to rotate, swipe down to soft/hard drop. `touch-action` defaults disabled on the canvas to prevent browser scroll/zoom gestures.
...
- **Idle mission recovery (PR #64).** Ship and crew availability now correctly resets to `On Station` / `Available` when an idle mission completes or is aborted, preventing permanently-deployed ghost assets.
- **Game UI leak on boot (PR #54).** The puzzle HUD no longer flashes behind the missions screen when the hub first loads.
- **Scaled click mapping (PR #55).** Click-to-match coordinates now translate correctly through the scene-scale transform so matches fire on the right cell on mobile and scaled desktops.
- **Idle mission UI node leak (PR #61).** Detached idle-fleet rows are now destroyed instead of merely removed, preventing a slow memory leak on long sessions.
...
- **Mobile responsiveness pass for hub + gameplay surfaces.** The fixed 860×820 in-run HUD now viewport-fits via scene scaling so boards no longer clip on narrow screens; hub shell now scale-fits below its desktop minimum layout so columns/nav/modal stop overlapping on phones; touch gesture controls were added for gameplay input (swipe left/right move, swipe up rotate, swipe down soft/hard drop); and touch-action defaults are disabled on the page/canvas to prevent browser scroll/zoom gestures from stealing game input.
